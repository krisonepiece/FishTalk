	...
	<section id="container">
		...
		<section id="main-content">
			<section class="wrapper">
				...
				/* 查詢好友申請 */
				$query = "	SELECT	R.*, matching.*
							FROM 	member D, member R, matching
							WHERE	R.account = matching.account_A AND
									D.account = '$account' AND
									matching.account_B = D.account AND
									matching.accept = '0';";
				...
				if ( $count > 0 ) {
					...
					echo "<h4><i class='fa fa-angle-right'></i> 好友管理 (未審核)</h4>";
					echo "<hr>";
					echo "<thead>";
					echo "<tr>";
					...
					while ( $row = mysqli_fetch_assoc( $result ) ) {
						...
						echo "<td>";
						echo "<a class='btn btn-success btn-xs' href='php/Verify_Friend.php?number={$row['number']}'><i class='fa fa-check'></i></a>";
						echo "<a class='btn btn-danger btn-xs' href='php/Delete_Friend.php? number={$row['number']}&delete=1' onclick='return confirm(\"確定要刪除嗎？\");'><i class='fa fa-trash-o'></i></a>";
						echo "</td>";
						echo "</tr>";
					}
					echo "</tbody>";
				...
				$query = "	SELECT	R.*, matching.*
							FROM 	member D, member R, matching
							WHERE	R.account = matching.account_B AND
									D.account = '$account' AND
									matching.account_A = D.account AND
									matching.accept = '0';";
				$result = mysqli_query( $conn, $query );
				$count = mysqli_num_rows( $result );
								if ( $count > 0 ) {
					...
					echo "<div class='content-panel table-responsive'>";
					echo "<table class='table table-striped table-advance table-hover'>";
					echo "<h4><i class='fa fa-angle-right'></i> 好友管理 (等待對方審核)</h4>";
					echo "<hr>";
					echo "<thead>";
					echo "<tr>";
					echo "<th  width='150px'><i class='fa fa-user-circle'></i> 帳號</th>";
				...
				$query = "	SELECT	R.*, matching.*
							FROM 	member D, member R, matching
							WHERE	matching.accept = 1 AND
									((R.account = matching.account_A AND
									D.account = '$account' AND
									matching.account_B = D.account) OR
									(R.account = matching.account_B AND
									D.account = '$account' AND
									matching.account_A = D.account));";
				$result = mysqli_query( $conn, $query );
				$count = mysqli_num_rows( $result );
				...
				echo "<div class='row mt'>";
					...
					echo "<div class='content-panel table-responsive'>";
					echo "<table class='table table-striped table-advance table-hover'>";
					echo "<h4><i class='fa fa-angle-right'></i> 好友名單</h4>";
					echo "<hr>";
					echo "<thead>";
					echo "<tr>";
					echo "<th  width='150px'><i class='fa fa-user-circle'></i> 帳號</th>";
				...
				if ( $count > 0 ) {
					...
					while ( $row = mysqli_fetch_assoc( $result ) ) {
						...
						echo "<td>{$row['username']}</td>";
						echo "<td>";
						echo "<a class='btn btn-danger btn-xs' href='php/Delete_Friend.php? number={$row['number']}&delete=0' onclick='return confirm(\"確定要刪除嗎？\");'><i class='fa fa-trash-o'></i></a>";
						echo "</td>";
						echo "</tr>";
					}
					echo "</tbody>";
